...
| `CONTESTS_CHECKOUT_APP_ORIGIN` | Express — public **browser** origin for Stripe Checkout **success/cancel** URLs (Phase 5 Story P5-D1) and Connect Account Link return URLs (Phase 6). Example: `http://localhost:4300` when using `ng serve`; production: **`https://…`** canonical host. **No trailing slash.** | `.env` / Cloud Run (Story **GL-D3:** [weekly-contests-gl-d3-checkout-redirect-origin.md](../weekly-contests/weekly-contests-gl-d3-checkout-redirect-origin.md)) |
...
**Webhooks (Story GL-D2):** set **`STRIPE_WEBHOOK_SECRET`** to the signing secret for your **live** webhook endpoint in Stripe Dashboard; **`GET /health`** includes **`stripeWebhookSecretConfigured`**. See [weekly-contests-gl-d2-stripe-webhook-live.md](../weekly-contests/weekly-contests-gl-d2-stripe-webhook-live.md).
