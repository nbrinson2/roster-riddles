...
| `POST` | `/api/v1/admin/contests/:contestId/payout-hold` | Phase 6 **P6-G2** — set **`prizePayoutStatus: held`** + hold metadata; blocks prize **execute** until resume. Body: **`{ "force": true, "reason": "…" }`**. Appends **ledger** audit line. |
| `POST` | `/api/v1/admin/contests/:contestId/payout-resume` | Phase 6 **P6-G2** — clear hold; **`prizePayoutStatus` → `scheduled`**. Body: **`{ "force": true, "reason"?: "…" }`**. Appends **ledger** audit line. |
| `POST` | `/api/v1/admin/contests/:contestId/payout-retry-failed` | Phase 6 **P6-G2** — retry **`failed`** payout lines on **`payouts/final`** (Stripe idempotent **`tr_…`** create); optional **`rank`** / **`uid`** to scope one line. Body: **`{ "force": true, "reason": "…", "rank"?: number, "uid"?: string }`**. Requires **`CONTESTS_PAYMENTS_ENABLED`**. |
...
Implementation: [`server/admin/admin-contests.http.js`](../server/admin/admin-contests.http.js), [`server/admin/admin-payouts.http.js`](../server/admin/admin-payouts.http.js) (P6-G1 read + P6-G2 mutations), [`server/contests/contest-payout-admin-actions.job.js`](../server/contests/contest-payout-admin-actions.job.js) (P6-G2), [`server/admin/admin-users.http.js`](../server/admin/admin-users.http.js), [`server/middleware/require-admin.js`](../server/middleware/require-admin.js), shared transition runner [`server/contests/contest-transition-run.js`](../server/contests/contest-transition-run.js).
