...
- **`stripePublishableKey`:** not required for Checkout redirect (no Stripe.js on the client). Set via **`STRIPE_PUBLISHABLE_KEY`** at CI for **`pk_live_…`** / **`pk_test_…`** when client Stripe is used — [weekly-contests-gl-c3-stripe-publishable-key-prod-bundle.md](weekly-contests-gl-c3-stripe-publishable-key-prod-bundle.md) (Story **GL-C3**).
