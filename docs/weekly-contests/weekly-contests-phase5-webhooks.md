...
**Transfers (Connect / platform → connected account):** Stripe’s [event types](https://docs.stripe.com/api/events/types) list only **`transfer.created`**, **`transfer.updated`**, and **`transfer.reversed`**. There is **no** `transfer.paid` or `transfer.failed` in the Dashboard picker — treat **`transfer.updated`** (and **`transfer.created`**) as carrying paid vs failed state via the **Transfer object** fields (`failure_code`, `reversed`, `amount_reversed`, etc.). The server still recognizes the legacy names **`transfer.paid`** / **`transfer.failed`** if Stripe ever emits them.

| Stripe `type` | When handled |
|---------------|----------------|
| **`transfer.created`**, **`transfer.updated`** | When Transfer **`metadata`** includes **`contest_id`**, **`firebase_uid`**, and (optionally) **`rank`** (same keys as [payout execute](../server/contests/contest-payout-execute.job.js) metadata): merge **`lastStripeEventId`** on the matching row in **`contests/{contestId}/payouts/final`**, set line **`status`** to **`succeeded`** for positive money lines when the object shows no failure/reversal, or **`failed`** with a **`failurePublicCode`** enum when **`failure_code`** / reversal flags apply; recompute **`aggregateStatus`**. |
| **`transfer.reversed`** | Same metadata gate: mark line **`failed`**, append **`ledgerEntries/prize_reversal_{tr_…}`** with **`prize_transfer_reversal`** (**`credit`**) for the reversed cents **once per Transfer** (so `transfer.updated` with `reversed` / `amount_reversed` does not double-credit; duplicate `evt_…` still idempotent via **`processedStripeEvents`**). |
| **`payout.paid`**, **`payout.failed`**, **`payout.canceled`**, **`payout.updated`** | Always write **`processedStripeEvents/{event.id}`** (idempotency). When **exactly one** `users/{uid}` has **`stripeConnectAccountId === event.account`**, merge optional **`stripePayoutLast*`** fields ([schema](weekly-contests-schema-users-payouts.md)). Optional: **`payout.created`**, **`payout.reconciliation_completed`** if you want full payout lifecycle in logs — not required for v1 user mirror logic. |
