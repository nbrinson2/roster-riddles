...
export function applyFeedbackColors(
  ...
  const targetYearSet = buildYearSet(target.groups);
...
export function getYearColor(
  ...
  // build the array of years for this group
  const groupYears = Array.from(
    { length: groupTo - groupFrom + 1 },
    (_, i) => groupFrom + i
  );

  // 1) fully contained → BLUE
  if (groupYears.every(y => targetYears.has(y))) {
    return PlayerAttrColor.BLUE;
  }

  // 2) partial overlap → ORANGE
  if (groupYears.some(y => targetYears.has(y))) {
