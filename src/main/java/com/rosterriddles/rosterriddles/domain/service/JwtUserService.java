...
public class JwtUserService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsernameNoLock(String username) throws UsernameNotFoundException {
        try {
            HashMap<String, Object> options = checkUser(username, false);
            boolean enabled = (boolean) options.get("enabled");
            boolean account_expired = (boolean) options.get("account_expired");
            boolean account_locked = (boolean) options.get("account_locked");
            boolean credentials_expired = (boolean) options.get("credentials_expired");
            UserJwtDetails userDetails = new UserJwtDetails(
                    username,
                    (String) options.get("password"),
                    enabled,
                    !credentials_expired,
                    ...
                    !account_locked,
                    new ArrayList<>());
            userDetails.setUserID((int) options.get("user_id"));
            return userDetails;
        } catch (Exception e) {
            e.printStackTrace();
            logger.debug("loadUserByNameException", e.getMessage());
