...
**Fix:** In [Cloud Build → Triggers](https://console.cloud.google.com/cloud-build/triggers), open the trigger that builds this repo and add **User-defined substitutions** for every `_FIREBASE_*` key (same values as your Firebase web app / local prod env). Ensure the trigger uses **this** `cloudbuild.yaml` (not a minimal autogenerated config that omits `--build-arg`).

If you use **Cloud Run “deploy from source”** with an implicit build, confirm it is wired to a trigger that supplies those substitutions; otherwise switch to a trigger that runs the repo’s `cloudbuild.yaml`.
